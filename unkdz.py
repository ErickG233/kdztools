...
class KDZFileTools(kdz.KDZFile):
    ...
    def readKDZHeader(self):
        ...
        for key in self._dz_collapsibles:
            if type(kdz_item[key]) is str or type(kdz_item[key]) is bytes:
                ...
                if kdz_item[key] != 0:
                    print('[!] Error: field "'+key+'" is non-zero (' +
                          b2a_hex(kdz_item[key])+')', file=sys.stderr)
                    sys.exit(1)
            else:
    ...
    def getPartitions(self):
        ...
        if len(buf.lstrip(b'\x00')) > 0:
            print("[!] Warning: Data between headers and payload! (offsets {:d} to {:d})".format(
                self.headerEnd, self.dataStart), file=sys.stderr)
            self.hasExtra = True
    ...
    def extractPartition(self, index):
        ...
        outfile = open(os.path.join(
            self.outdir, currentPartition['name'].decode("utf8")), 'wb')
        ...
        while True:
            ...
            if outfile.tell() + chunkSize >= currentPartition['length']:
                ...
                outfile.write(self.infile.read(
                    currentPartition['length'] - outfile.tell()))
                # Prevent runaways!
                break
    ...
    def saveParams(self):
        ...
        params.write(
            '# saved parameters from the file "{:s}"\n'.format(self.kdzfile))
    ...
    def parseArgs(self):
        ...
        parser.add_argument('-f', '--file', help='KDZ File to read',
                            action='store', required=True, dest='kdzfile')
        group = parser.add_mutually_exclusive_group(required=True)
        group.add_argument('-l', '--list', help='list partitions',
                           action='store_true', dest='listOnly')
        group.add_argument('-x', '--extract', help='extract all partitions',
                           action='store_true', dest='extractAll')
        group.add_argument('-s', '--single', help='single Extract by ID',
                           action='store', dest='extractID', type=int)
        parser.add_argument('-d', '--dir', '-o', '--out',
                            help='output directory', action='store', dest='outdir')
    ...
    def openFile(self, kdzfile):
        ...
        if verify_header not in self.kdz_header:
            ...
            print('[ ] Received header "{:s}".'.format(
                " ".join(b2a_hex(n) for n in verify_header)))
            sys.exit(1)
    ...
    def cmdExtractSingle(self, partID):
        print(
            "[+] Extracting single partition from v{:d} file!\n".format(self.header_type))
        print("[+] Extracting " + str(self.partList[partID][0]) + " to " +
              os.path.join(self.outdir, self.partList[partID][0].decode("utf8")))
        self.extractPartition(partID)

    def cmdExtractAll(self):
        print(
            "[+] Extracting all partitions from v{:d} file!\n".format(self.header_type))
        for part in enumerate(self.partList):
            print("[+] Extracting " + part[1][0].decode("utf8") + " to " +
    ...
    def cmdListPartitions(self):
        ...
        for part in enumerate(self.partList):
            print("{:2d} : {:s} ({:d} bytes)".format(
                part[0], part[1][0].decode("utf8"), part[1][1]))
    ...
    def main(self):
        ...
        if args.listOnly:
            ...
            if args.extractID >= 0 and args.extractID < len(self.partList):
                ...
                print("[!] Segment {:d} is out of range!".format(
                    args.extractID), file=sys.stderr)
